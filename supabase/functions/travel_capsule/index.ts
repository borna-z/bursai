...
import {
  ...
  buildPlanningRequirementsText,
  CREATE_TRAVEL_CAPSULE_TOOL,
  formatWardrobeLines,
  resolveGarmentId,
  hydrateGarment,
  LOCALE_NAMES,
  ...
  type GarmentRow,
  type PlannedCapsuleOutfit,
  type ScoredGarment,
} from "../_shared/travel-capsule-helpers.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    const mustHaveNote = mustHaveIds.length > 0
      ...
      : "";
    const planningRequirements = buildPlanningRequirements(planningSlots);
    ...
    const planningRequirementsText = buildPlanningRequirementsText(planningRequirements, dayOccasionLabels);
...
Write all text content (notes, tips, reasoning) in ${LOCALE_NAMES[locale] || "English"}.`;
    ...
    const tools = [CREATE_TRAVEL_CAPSULE_TOOL];
    ...
    // ─────────────────────────────────────────────
    // BANNED_NOTE_PATTERNS lives in _shared/travel-capsule-helpers.ts
