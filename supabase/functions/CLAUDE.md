...
| `outfit-scoring-body.ts` / `outfit-scoring-color.ts` | Sub-modules of `outfit-scoring.ts`. The parent module imports the symbols it needs directly AND re-exports both with `export *`, so downstream functions (`burs_style_engine`, etc.) only import the main `outfit-scoring.ts` barrel. Don't add direct imports to these sub-files from outside `_shared/`. |
...
| `gemini-cache.ts` | Wave S-B (2026-05-15): wrapper around Gemini's `cachedContent` API — single public entrypoint `ensureCachedContent(config)` returns a cache id, creating + refreshing as needed (`_clearCacheForTesting` is test-only). Long static prompt prefixes (e.g. the 600-token enrich-mode taxonomy in `analyze_garment`) are cached upstream and reused by ID rather than re-sent. Cache TTL refreshes on a 24h schedule. |
| `render-eligibility.ts` | Gemini vision gate — determines if garment images need ghost-mannequin rendering. |
