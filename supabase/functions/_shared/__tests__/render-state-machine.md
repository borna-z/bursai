...
**I12 — Garment delete never orphans an active reservation.** Round 12 added this invariant. `render_jobs.garment_id REFERENCES garments(id) ON DELETE CASCADE`, so deleting a garment wipes its render_jobs rows. `render_credit_transactions` has no FK to `render_jobs` by design (the ledger survives job-row deletion for historical reasons), so a naive `DELETE FROM garments WHERE id=X` leaves `reserve` transactions for X's jobs with no matching render_jobs row — the user's `render_credits.reserved` counter stays elevated forever because the worker can never see the (now-deleted) job to terminalize it. Every delete path (UI `useDeleteGarment`, `AddGarment.tsx` duplicate-replace, `seed_wardrobe` bulk wipe) MUST call `release_reservations_for_garment_delete(p_garment_id)` before issuing the DELETE. The RPC is SECURITY DEFINER with an ownership check (auth.uid() must match garment.user_id, or caller is service_role); it writes release transactions for every non-terminal render_jobs row belonging to the garment. Release failures are logged but do NOT block the delete — the post-launch orphan-reservation cleanup cron is the safety net for the narrow error window.
...
### Scenario 15 — Garment delete with an active (non-terminal) render

**Trigger:** User deletes a garment while a render_jobs row for that garment is still in `pending` / `in_progress` status. Could be:
- User enqueued a render, then changed their mind and tapped delete.
- User tapped Regenerate, then deleted before the queue processed it.
- Duplicate-replace flow on AddGarment (`onReplace` deletes the old garment).
- `seed_wardrobe` bulk wipe while user has in-flight renders.
...
**Action (post-round-12):**

1. Caller (useDeleteGarment / AddGarment onReplace / seed_wardrobe loop) invokes `supabase.rpc('release_reservations_for_garment_delete', { p_garment_id: id })`.
2. RPC runs as SECURITY DEFINER: verifies ownership (auth.uid() == garment.user_id, or caller is service_role). For each non-terminal render_jobs row for this garment: looks up the reserve tx source, decrements `render_credits.reserved` (refunds to the matching source column — trial_gift_remaining / topup_balance / or leaves monthly unchanged), writes a `release` tx with idempotency_key `release:garment_delete:<job_id>`. Returns the released count.
3. Caller proceeds with `DELETE FROM garments WHERE id=X` — the CASCADE wipes render_jobs rows, but the ledger is already balanced.

**Post:** Garment gone. render_jobs rows for that garment gone (cascade). Credit txs for each job: `{reserve, release}`. `render_credits.reserved` decremented back to its pre-reserve value. No orphaned reservations.

**Failure modes:**

- RPC returns an error or throws → caller logs and proceeds with delete anyway. Reservations stay orphaned for this delete path. Post-launch orphan-cleanup cron sweeps them.
- Two concurrent delete calls for the same garment → RPC is idempotent (ON CONFLICT on `release:garment_delete:<job_id>` idempotency_key). Only one release tx written per job.
- Delete happens via admin SQL without going through the RPC → orphaned reservations. Post-launch cron is the safety net. **This is the primary motivation for treating the orphan cleanup cron as a launch-blocker-adjacent follow-up**, not a routine post-launch nice-to-have.

**Invariants:** I1 ✓ (single reserve per job). I2 ✓ (single terminal = release per job). **I12 ✓ NEW**.
