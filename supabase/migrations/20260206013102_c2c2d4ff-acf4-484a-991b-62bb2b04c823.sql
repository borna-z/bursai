...
DROP POLICY IF EXISTS "Users can view own planned outfits" ON public.planned_outfits;
CREATE POLICY "Users can view own planned outfits"
ON public.planned_outfits
FOR SELECT
USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own planned outfits" ON public.planned_outfits;
CREATE POLICY "Users can insert own planned outfits"
ON public.planned_outfits
FOR INSERT
WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own planned outfits" ON public.planned_outfits;
CREATE POLICY "Users can update own planned outfits"
ON public.planned_outfits
FOR UPDATE
USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete own planned outfits" ON public.planned_outfits;
...
CREATE INDEX IF NOT EXISTS idx_planned_outfits_user_date ON public.planned_outfits(user_id, date);
