-- Wave 8.5 PR B audit Round 7 — fixup for migration 20260502120700.
--
-- The ownership-check SQL used `SELECT DISTINCT g FROM unnest(...) g` which
-- in PG returns record-type rows, not the underlying uuid column. Building
-- an array out of records produced `record[]`, and `id = ANY(record[])`
-- never matched the `uuid` ids in `garments` — so the ownership check
-- ALWAYS failed (counted 0 owned, but supplied N), raising 42501 on every
-- legitimate save_outfit call. Surfaced by the smoke test on commit
-- 7def2c45 (post-field-name fix): happy path returned 403 ownership_denied
-- even when the test seeded user-owned garments.
--
-- Fix: explicit column alias `AS u(id)` so the array aggregates uuid
-- values, not records. Same `CREATE OR REPLACE FUNCTION` shape as 120700
-- (single statement, no prepared-statement parser issues).
...
BEGIN
  ...
  -- ----- Ownership validation (Round 6 R6-1 + Round 7 fix) -----------------
  -- The 120700 version of this block used `SELECT DISTINCT g FROM unnest(arr) g`
  -- which returned record rows, not uuids — `id = ANY(record[])` never
  -- matched, breaking happy-path saves. Use explicit `AS u(id)` alias so the
  -- subquery returns a uuid column.
