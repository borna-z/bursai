...
- Added 7 new smoke tests: `enrichment`, `render`, `outfit-generate`, `outfit-refine`, `visual-search`, `shopping-chat`, `travel-capsule`. Total suite now 10. All 10 pass locally against fresh `supabase db reset` (Strategy V baseline) and green in both `smoke-local` and `smoke-prod` CI jobs.
- Scope choice mirrors the existing 3 tests: DB-level shape validation using the authed anon client + RLS. `analyze_garment`, `render_garment_image`, `generate_outfit`, `style_chat`, `visual_search`, `shopping_chat`, `travel_capsule` edge functions are NOT invoked because `GEMINI_URL` is hardcoded in `supabase/functions/_shared/burs-ai.ts` and rerouting it to the mock server requires edge-function changes that are out-of-scope for this tests-only prompt. Each test exercises the persistence contract (column projection, CHECK constraints, RLS scoping, jsonb round-trip) that the corresponding edge function depends on — drift in those invariants is the exact regression mode the suite is meant to catch.
- Mock server scaffolding from P0d-ii remains dormant until a future prompt rearchitects the edge functions for testability (e.g. promote `GEMINI_URL` to an env var so local Supabase runtime can point it at the mock).
- Files: 7 new test files under `src/test/smoke/`. No production code touched.
- Deploy: none
...
| 2026-04-20 | P0d-iii | `supabase/functions/_shared/burs-ai.ts:17` | **`GEMINI_URL` is a hardcoded `const`**, not read from env. That makes edge-function Gemini calls impossible to reroute through the mock server booted by `src/test/smoke/mocks/mock-server.ts` — the whole reason P0d-ii's mock infrastructure exists. Surfaced when writing P0d-iii smoke tests: to hit `analyze_garment` / `render_garment_image` / etc. under local Supabase with a mocked Gemini, the edge-runtime needs `GEMINI_URL` overridable via `Deno.env.get("GEMINI_URL") ?? "https://generativelanguage..."`. P0d-iii tests instead validate DB shapes directly (matching the P0d POC pattern), but a future prompt should promote this URL to an env var so the 7 dormant mock routes in `mocks/gemini.ts` can actually be wired up and smoke coverage can extend to edge-function behaviour. | Dedicated follow-up: promote `GEMINI_URL` to env-var in `supabase/functions/_shared/burs-ai.ts`, add `GEMINI_URL` to local function runtime env wiring in `supabase/functions/_shared/gemini-image-client.ts` if it has the same pattern, then revisit P0d-iii tests to actually invoke edge functions under local Supabase with mocks. One-line edge-function change, needs redeploy across all AI functions. |
...
| 2026-04-20 | #640 | P0d-iii | Smoke-test suite expanded from 3 to 10. Added enrichment, render, outfit-generate, outfit-refine, visual-search, shopping-chat, travel-capsule — DB-level shape validation mirroring the P0d POC pattern. Edge function code NOT touched (hardcoded `GEMINI_URL` precluded mock wiring without out-of-scope edge-function changes). |
