...
**P37 [DONE] (PR #682, 2026-04-25)** Localize MoodOutfit MOODS hint strings
- Frontend-only scope per minimum-change principle (the spec's MOOD_MAP entry stays English — it's AI direction, AI handles localized output via existing langName from P32). 12 hardcoded English `hint:` strings in `MOODS` const at `src/pages/MoodOutfit.tsx:25-38` ("Sharp. Owned.", "Warm. Enveloping.", etc.) replaced with `hintKey: 'mood.hint.{key}'` referencing translation keys; JSX render-site at line 389 changed from `{mood.hint}` to `{t(mood.hintKey)}`. **24 new keys** appended to en.ts + sv.ts (12 each, append-only).
- Files: `src/pages/MoodOutfit.tsx`, `src/i18n/locales/en.ts`, `src/i18n/locales/sv.ts`
- Deploy: none (frontend-only, Vercel auto-deploys)

**P38 [DONE] (PR #682, 2026-04-25)** Extend token lists to all 14 locales
- 3 shared classifier modules previously had English + Swedish only — extended with garment vocabulary across no/da/fi/de/fr/es/it/pt/nl/pl/ar/fa. (a) `_shared/burs-slots.ts` — TOP/BOTTOM/SHOES/OUTERWEAR/DRESS/ACCESSORY arrays. (b) `_shared/outfit-rules.ts` — same 5 arrays + BASE_LAYER_TOKENS + MID_LAYER_TOKENS + ~95 new EXPLICIT_SLOT_MAP entries. (c) `_shared/travel-capsule-planner.ts` — same 5 arrays. ASCII fallback variants added in outfit-rules.ts + travel-capsule-planner.ts (those don't strip accents at match time, unlike burs-slots.ts which DOES). **Cross-language collisions skipped**: ES "saco" (jacket) vs PT "saco" (bag); NL "rok" (skirt) substring of SV "rock" (coat); DE "hut"/PL "pas" too short; FA "ژاکت"/"کت" overlap; DE "strickjacke" substring contains "jacke" (OUTERWEAR fires first, would clip MID_LAYER). Pre-existing collision documented for future cleanup: `rock` is sv/de=coat AND English "rock band" subcategories — pre-P38 problem, not new. Translator-pass needed (Findings Log).
- Files: 3 shared modules
- Deploy: consumers — `mood_outfit`, `generate_outfit`, `clone_outfit_dna`, `travel_capsule` (burs-slots + outfit-validation chain), `style_chat` (outfit-validation), `summarize_day` (none from these three but bundles into P39 deploy radius)

**P39 [DONE] (PR #682, 2026-04-25)** Localize day-intelligence.ts OCCASION_RULES
- All 8 OCCASION_RULES tags arrays extended with native-language keywords across 13 additional locales (sv + no/da/fi/de/fr/es/it/pt/nl/pl/ar/fa). **Real accented forms used** (möte, häät, télétravail) since `normalizeText` only `.toLowerCase()`s — no accent strip — different from burs-slots.ts which DOES strip. Tag growth per rule: formal 6→35, party 5→39, work 7→70, dinner 5→38, travel 9→70, workout 7→47, social 5→41, remote 4→33. Inline arrays (lines 122-124) `travelTagged`/`workoutTagged`/`outdoorTagged` also extended (47/47/32 tokens respectively). **Cross-language collisions skipped**: FR "course" (academic class collision), FR "vol" (3-char "Vol. 2" abbrev collision), PL "lot" (3-char English "lot" collision). Translator-pass needed.
- Files: `supabase/functions/_shared/day-intelligence.ts`
- Deploy: `summarize_day` (sole consumer per grep)
...
**P42 [DONE] (PR #683, 2026-04-26)** Migration: 4 new profiles columns
- Shipped as part of Wave 7 PR 1 (P42+P43+P44 bundle per approved 5-PR phasing). New migration `supabase/migrations/20260426120000_onboarding_state.sql` adds 4 columns on `public.profiles` (`onboarding_step text NOT NULL DEFAULT 'not_started'`, `onboarding_garment_count integer NOT NULL DEFAULT 0`, `onboarding_started_at timestamptz`, `onboarding_completed_at timestamptz`), a CHECK constraint pinning the 10-value step enum (`not_started → language → quiz → photo_tutorial → batch_capture → achievement → studio_selection → coach_tour → reveal → completed`), a partial index `idx_profiles_onboarding_step` on `(onboarding_step) WHERE onboarding_step <> 'completed'` (most users will be 'completed' over time so a full index would waste space — only the "still onboarding" set is queried), and 2 SECURITY DEFINER RPCs: `advance_onboarding_step(p_user_id, p_to_step)` (forward-only state machine, returns jsonb, sets started_at on first transition out of 'not_started', sets completed_at when stepping to 'completed', rejects no-op + backwards moves) and `increment_onboarding_garment_count(p_user_id)` (atomic counter increment, returns new count). Both enforce ownership (`auth.uid() = p_user_id`) OR service-role caller. Backfill: existing rows where `(preferences->'onboarding'->>'completed')::boolean IS TRUE` get `step='completed'` + `completed_at = COALESCE(updated_at, NOW())`; idempotent via `WHERE onboarding_step <> 'completed'` guard. Wrapped in `BEGIN/COMMIT` so partial failures roll back cleanly. Migration follows the OAuth CSRF + grant_trial_gift_atomic patterns (`SET search_path = public`, `REVOKE ALL ... FROM PUBLIC`, `GRANT EXECUTE ... TO authenticated, service_role`). Migration drift check pre-merge clean (`supabase migration list --linked` shows new file Local-only; `db push --linked --dry-run` lists only the new migration as pending).
- Files: `supabase/migrations/20260426120000_onboarding_state.sql` (new)
- Deploy: post-merge `npx supabase db push --linked --yes`. Then regenerate `src/integrations/supabase/types.ts` via `supabase gen types typescript --linked` (auto-generated file per CLAUDE.md hard rule — intentionally stale in this PR; ProtectedRoute uses a `(profile as { onboarding_step?: string | null })` cast as a transitional safety; cast can be removed in PR 2 after types.ts regen lands).

**P43 [DONE] (PR #683, 2026-04-26)** Onboarding rate-limit boost in scale-guard
- Shipped bundled with P42+P44. (a) `SubscriptionPlan` union widened from `"free" | "premium"` to `"free" | "premium" | "onboarding"`. (b) `TIER_MULTIPLIERS` gained `onboarding: 3.0` (free=0.75x, premium=2.0x, onboarding=3.0x). (c) New `ONBOARDING_BOOST_WINDOW_MS = 24 * 60 * 60 * 1000` constant. (d) `resolveUserPlan()` rewritten to query `subscriptions` AND `profiles` in parallel (`.single()` returns `{data, error: PGRST116}` for 0 rows, doesn't throw — missing profile/subscription falls through cleanly). When `profile.onboarding_started_at` is non-null, `onboarding_step !== 'completed'`, and `Date.now() - startedMs < 24h`, the resolved plan is `"onboarding"` — outranks subscription tier. Wave 8 forward-compat: when free tier is removed, the override logic doesn't change — only the `TIER_MULTIPLIERS` values change. (e) `Number.isFinite(startedMs)` coerces malformed `onboarding_started_at` strings safely (NaN → falls through to subscription). (f) Per-isolate cache (5min TTL) preserved; a user crossing the 24h window mid-cache may stay boosted for up to 5 extra minutes — acceptable, the boost is generous, not abusive. (g) **Test-only exports** added to enable unit-testing without mocking the full enforceRateLimit pipeline: `resolveUserPlan` and `applyTierMultiplier` (was internal) + `__resetSubscriptionCacheForTests` helper. Public consumers outside scale-guard.ts and its tests should not import `resolveUserPlan` — the contract is "called by enforceRateLimit, output cached per-isolate". (h) Updated `enforceRateLimit` docstring to list all 3 tiers. New test file `_shared/__tests__/scale-guard.test.ts` (19 tests covering: applyTierMultiplier for all 3 tiers + min-1 floor + analyze_garment composition; onboarding boost active/expired/completed/null-started/malformed-date/no-profile/both-missing/DB-throws; subscription tier resolution active/trialing/cancelled/free regression net; per-isolate cache reuses on second call). All 19 pass.
- Files: `supabase/functions/_shared/scale-guard.ts`, `supabase/functions/_shared/__tests__/scale-guard.test.ts` (new)
- Deploy: per Shared Module Deploy Map, `_shared/scale-guard.ts` change requires redeploying ALL 22 AI functions (post-merge, ONE AT A TIME per CLAUDE.md hard rule).

**P44 [DONE] (PR #683, 2026-04-26)** Route gate enforcement
- Shipped bundled with P42+P43. (a) Source of truth swapped from `prefs?.onboarding?.completed === true` to `(profile as { onboarding_step?: string | null }).onboarding_step !== 'completed'`. Cast guards against types.ts being temporarily stale (auto-generated; regen runs post-merge). (b) Single `skipOnboardingCheck?: boolean` prop removed from `ProtectedRouteProps`. Callers no longer need to opt out — gate is pathname-driven. (c) New `src/components/auth/onboardingExempt.ts` module exports `ONBOARDING_EXEMPT_PATHS` (8 paths: `/onboarding`, `/billing`, `/auth`, `/admin`, `/privacy`, `/terms`, `/share`, `/u`) + `isOnboardingExempt(pathname)` matcher (exact match OR `${path}/...` prefix — `/uniform` does NOT match `/u`, `/authenticated` does NOT match `/auth`). Defensive entries for `/admin`, `/privacy`, `/terms` aren't currently mounted as routes — listing them prevents future deep-links from accidentally trapping non-completed users. (d) **Lockstep changes**: `AnimatedRoutes.tsx:125` removed `skipOnboardingCheck` prop on `/onboarding` route (now exempt by pathname); `AnimatedRoutes.discoverRedirect.test.tsx:15` mock typing updated from `PropsWithChildren<{ skipOnboardingCheck?: boolean }>` → `PropsWithChildren`. (e) **Module split (lint fix)**: helpers were initially in `ProtectedRoute.tsx` but ESLint's `react-refresh/only-export-components` flagged the file for exporting non-component values. Split into `onboardingExempt.ts` (pure helpers) + `ProtectedRoute.tsx` (component-only). Spec acceptance preserved. (f) Tests: `ProtectedRoute.test.tsx` rewrite — 16 tests covering auth gate (login redirect, loading spinner for auth + profile load), onboarding gate (completed lets through; not_started/mid-flow redirect; missing onboarding_step redirects defensively; exempt paths /onboarding /share/abc-123 /billing/success /u/borna pass through), `isOnboardingExempt` matcher (exact match each of 8 paths; sub-path match; prefix-collision rejection like /uniform vs /u; unrelated routes; root pathname). Test setup uses route declaration order to handle path collisions (protected route before redirect-target route).
- (g) **Scope expansion (Fix Protocol exception (b) — rollout bridge)**: code-reviewer subagent flagged that `src/hooks/useOnboarding.ts:17-25` `completeOnboarding()` only writes the legacy `preferences.onboarding.completed=true` flag. Without intervention, users finishing the OLD onboarding flow between PR 1 merge and PR 2 (P45) ship would write the legacy flag (ignored by new gate) AND keep `onboarding_step='not_started'` (default, gate redirects them to /onboarding indefinitely — trapped). Fix: `completeOnboarding()` now ALSO calls `supabase.rpc('advance_onboarding_step', { p_user_id, p_to_step: 'completed' })` BEFORE the legacy preferences write. RPC error → throw + skip legacy write. RPC `{ok:false}` (no-op for already-completed users) → tolerate + still write legacy flag. Test file `useOnboarding.test.tsx` updated (4 new tests: RPC called with right args; legacy write happens after RPC; RPC error throws + blocks legacy write; ok:false tolerated). `as any` cast used for `supabase.rpc` because types.ts is auto-generated (CLAUDE.md hard rule) and the new RPC isn't in the typed Functions union until post-merge regen.
- Files: `src/components/auth/ProtectedRoute.tsx`, `src/components/auth/onboardingExempt.ts` (new), `src/components/auth/__tests__/ProtectedRoute.test.tsx`, `src/components/layout/AnimatedRoutes.tsx`, `src/components/layout/__tests__/AnimatedRoutes.discoverRedirect.test.tsx`, `src/hooks/useOnboarding.ts`, `src/hooks/__tests__/useOnboarding.test.tsx`
- Deploy: none (frontend-only, Vercel auto-deploys on merge)
