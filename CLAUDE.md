...
**P27a [TODO]** Schema: nullable `secondary_image_path` on garments
- New migration adds `secondary_image_path TEXT` (nullable, no default, no CHECK) to `public.garments`. Name follows existing `original_image_path` / `processed_image_path` / `rendered_image_path` pattern. Storage reuses existing `garments` bucket with path convention `${userId}/${garmentId}_secondary.jpg`. No RPC needed — swap is one atomic `UPDATE garments SET image_path = secondary_image_path, secondary_image_path = image_path WHERE id = $1 AND user_id = auth.uid()`. No backfill.
- Files: new migration `supabase/migrations/<ts>_garments_secondary_image.sql`
- Deploy: none (post-merge `npx supabase db push --linked --yes`)

**P27b [TODO]** Secondary image flow: add, swap primary, delete (GarmentDetail only — post-save)
- Entry point is `GarmentDetail.tsx` ONLY. No touchpoint in AddGarment, LiveScan capture, or onboarding. Visible only when garment exists, i.e. after it has been saved and (typically) rendered. NOT available while `render_status === 'pending' | 'rendering'` or `enrichment_status === 'processing' | 'in_progress'` — "Wait for current enrichment/render" toast; prevents racing the worker.
- **Add secondary**: reuses the existing LiveScan capture component (no Median-hook changes — `useMedianCamera.ts` / `useMedianStatusBar.ts` / `src/lib/median.ts` stay frozen until Wave 9). Uploads to `garments/${userId}/${garmentId}_secondary.jpg`. Single UPDATE writes `secondary_image_path`. Triggers NOTHING in the AI pipeline — secondary is not primary yet.
- **Swap primary**: "Use this as primary" button on the secondary preview. Does:
  1. Atomic swap: `UPDATE garments SET image_path = secondary_image_path, secondary_image_path = image_path WHERE id = $1 AND user_id = $2` (RLS enforces ownership; single statement so no intermediate state).
  2. Reset enrichment: set `enrichment_status = 'pending'`, NULL `ai_raw` / `ai_analyzed_at` / `ai_provider` and derived columns (`silhouette`, `visual_weight`, `texture_intensity`, `style_archetype`, `occasion_tags`, `versatility_score`).
  3. Reset render: set `render_status = 'pending'`, NULL `rendered_image_path` / `rendered_at` / `render_error`.
  4. Enqueue fresh enrichment + render jobs via the same client-side paths AddGarment uses today (NOT a new edge function). Render consumes one render credit — documented as expected behavior in PR body.
- **Delete secondary**: "Remove" button. Nulls `secondary_image_path` + deletes the storage object via Supabase storage API. No AI re-trigger (secondary wasn't primary).
- UI: small thumbnail of secondary adjacent to primary in GarmentDetail hero; tap to expand; action buttons "Use as primary" and "Remove" below the expanded view. `hapticLight()` on every action.
- Files: `src/pages/GarmentDetail.tsx`, new `src/components/garment/SecondaryImageManager.tsx`, relevant enrichment + render enqueue hooks (verify whether existing `useEnqueueRenderJob.ts` + `src/lib/garmentIntelligence.ts` cover this or need a second entry point)
- Deploy: none (wardrobe + all AI functions continue reading `image_path` unchanged; no backend code modification)
...
**W4.9-A [TODO]** Schema cleanup — drop image_processing_* columns + remove outfit-scoring gate + delete orphan job_queue rows + backfill enrichment_status spelling
- Drops 6 `image_processing_*` columns from `garments` (P15 Finding). Removes `_shared/outfit-scoring-body.ts:467` gate that reads them (P15 Finding F2). Deletes orphan `job_queue` rows where `job_type = 'image_processing'` (P15 Finding F3). Backfills `enrichment_status`: `'complete'` → `'completed'`, `'in_progress'` → `'processing'` (P24 Finding). Updates 3 frontend writers to emit canonical values. Keeps `isEnrichmentReady` helper dual-spelling acceptance indefinitely.
- Files: 2 new migrations, `supabase/functions/_shared/outfit-scoring-body.ts:467`, `src/lib/garmentIntelligence.ts:437`, `src/pages/GarmentDetail.tsx:215`, `src/components/onboarding/QuickUploadStep.tsx:108`
- Deploy: `burs_style_engine` (only runtime consumer of outfit-scoring-body that this change affects)

**W4.9-B [TODO]** Docs + i18n drift — LAUNCH_PLAN.md P8 spec correction + drop 5 orphan avatar i18n keys
- Fix P8 spec's broken `cache_namespace` column reference (P8 Finding) with the TTL-decay mitigation text that actually shipped + pointer to L554 schema change. Delete 5 orphan `settings.avatar_*` keys from all 14 locale files (cleanup B Finding) — `ProfileCard.tsx` no longer calls any.
- Files: `LAUNCH_PLAN.md`, `src/i18n/locales/*.ts` (14 locale files)
- Deploy: none

**W4.9-C [TODO]** Observability — Sentry hook on validator_unavailable fail-open path
- Emit `Sentry.captureMessage('render_validator_unavailable', { level: 'warning', tags: { attempt: 1, category } })` when `validateRenderedGarmentOutputWithGemini` fails open on attempt 1 (Wave 3-B Finding #8). Aggregate validator outages become visible instead of silently degrading.
- Files: `supabase/functions/render_garment_image/index.ts`
- Deploy: `render_garment_image`

**W4.9-D [TODO]** P0c execution — verify/adjust Fix Protocol wording
- Executes P0c per Wave Closure Rule. Re-read Fix Protocol section; adjust wording where experience diverged (Launch Plan Update rule now lives inside Fix Protocol; code-reviewer agent call is post-code pre-push).
- Files: `CLAUDE.md`
- Deploy: none

**W4.9 user-action items (checkbox list inside the W4.9-A or W4.9-B PR body; track completion, not a code change):**
- [ ] Provision `SUPABASE_SERVICE_ROLE_KEY_TEST` GitHub repo secret (P0d-iv Finding — every push-to-main smoke-prod CI currently fails loud until this lands)
- [ ] `git merge --abort` in main repo + `git checkout main && git pull --ff-only` + prune unused worktrees via `git worktree remove` (P4 Finding)
- [ ] Supabase Dashboard → Edge Functions → Schedules audit for `calendar.handleSyncAll` caller; if dead, file mini-PR (P2 Finding)
