...
| 2026-04-21 | P8 | `LAUNCH_PLAN.md` P8 spec (lines 714-717) + CLAUDE.md prompt list | **Spec column-name error.** Spec's code snippet for `ai_response_cache` cleanup used `.like("cache_namespace", ...)` but that column doesn't exist on the table. Verified against the baseline migration (`supabase/migrations/00000000000000_initial_schema.sql` lines 732-741) — real columns are `id, cache_key, response, expires_at, created_at, model_used, hit_count, compressed`. Shipped with `.like("cache_key", ...)` which matches the actual column. Cache keys for user-scoped namespaces concatenate the user UUID per `_shared/burs-ai.ts` convention, so the LIKE still correctly drops this user's entries. **P13 follow-up** (Wave 2) will user-scope ALL cache namespaces, at which point the cleanup becomes exhaustive. Surfaced when reading the target file + schema dump before executing P8. | Fixed in-band in P8 PR. LAUNCH_PLAN.md P8 spec should be updated to use `cache_key` in a future doc-sweep pass (same pattern as the P1 spec correction in PR #649). Not opening a separate doc PR right now to avoid churn. |
...
| 2026-04-21 | #652 | P8 | Complete delete_user_account cascade: added DELETEs for 11 orphan-row tables (`render_credit_transactions`, `render_jobs`, `render_credits`, `feedback_signals`, `garment_pair_memory`, `analytics_events`, `ai_rate_limits`, `outfit_feedback`, `push_subscriptions`, `travel_capsules`, `ai_response_cache`). Spec said 12 but `chat_messages` was already in the existing cascade. Render FK order preserved (transactions→jobs→credits). Silent-delete pattern matches existing file convention for leaf cleanups. Used `cache_key` LIKE (not the spec's `cache_namespace` — column doesn't exist). GDPR right-to-erasure hygiene. Post-merge: deploy `delete_user_account`. |
