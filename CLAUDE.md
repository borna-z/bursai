...
**P9 [DONE] (PR #657, 2026-04-21)** Add rate limit + overload to 14 functions (Wave 2-A)
- Applied `enforceRateLimit` + `checkOverload` + `RateLimitError` catch to 10 user-facing functions: `import_garments_from_links`, `insights_dashboard`, `seed_wardrobe`, `send_push_notification`, `restore_subscription`, `create_portal_session`, `delete_user_account`, `calendar`, `google_calendar_auth`, `generate_outfit`. Applied `checkOverload` only to 3 cron-only functions: `daily_reminders`, `process_job_queue`, `cleanup_ai_cache`. `process_garment_image` skipped (being removed in P15). Total: 13 function deploys.
- Added 10 new `RATE_LIMIT_TIERS` entries in `_shared/scale-guard.ts` per spec (3/hr-60/hr range, per-function cost-aware).
- **Scope expansion**: `cleanup_ai_cache` had NO auth gate at all (P1 missed it). Added the same timingSafeEqual hard-reject pattern P1 shipped to `daily_reminders`/`process_job_queue`. Without this, P9's overload gate is toothless — an anon caller could still spam the DB-delete call.
- Files: 13 function `index.ts`, `_shared/scale-guard.ts`
...
**P11 [DONE] (PR #657, 2026-04-21)** Gate seed_wardrobe delete_all (Wave 2-A)
- Two-step flow: client POSTs `{action:"request_delete_token"}` to receive a 32-byte hex token with 5-min TTL, then POSTs `{action:"delete_all", confirmation:<token>}`. Token is consumed on first use (nulled in DB before the wipe so replay fails even if wipe partially errors).
- Also updated `SeedContext.tsx` (only in-repo caller) to use the two-step flow.
- Files: `supabase/functions/seed_wardrobe/index.ts`, `src/contexts/SeedContext.tsx`, new migration `supabase/migrations/20260421160000_seed_wardrobe_delete_confirmation.sql` (2 idempotent `ADD COLUMN IF NOT EXISTS` on `profiles`).
...
| 2026-04-21 | #657 | P9+P10+P11 | **Wave 2-A themed PR.** (a) P9 — overload + rate-limit swept across 13 functions. 10 user-facing (`import_garments_from_links`, `insights_dashboard`, `seed_wardrobe`, `send_push_notification`, `restore_subscription`, `create_portal_session`, `delete_user_account`, `calendar`, `google_calendar_auth`, `generate_outfit`) get the full `enforceRateLimit` + `checkOverload` + `RateLimitError` catch pattern. 3 cron-only (`daily_reminders`, `process_job_queue`, `cleanup_ai_cache`) get `checkOverload` only. 10 new `RATE_LIMIT_TIERS` entries in `_shared/scale-guard.ts` per spec. `process_garment_image` skipped (being removed in P15). **Scope expansion**: `cleanup_ai_cache` had NO auth gate at all (P1 missed it) — added the same `timingSafeEqual` hard-reject pattern already shipped to `daily_reminders`/`process_job_queue`. Without this, P9's overload gate is toothless. (b) P10 — `src/lib/validators.ts` (new) with `isUuid` + `isValidUsername`; applied in `PublicProfile.tsx` + `ShareOutfit.tsx` to short-circuit malformed URL params before any DB call. (c) P11 — seed_wardrobe destructive-op gate: new `request_delete_token` action returns a 32-byte hex token with 5-min TTL; `delete_all` now requires that token in the body and consumes it on use (nulled before wipe). Migration `20260421160000_seed_wardrobe_delete_confirmation.sql` adds `delete_confirmation_token` + `delete_confirmation_expires_at` to `profiles` (idempotent `ADD COLUMN IF NOT EXISTS`). `SeedContext.tsx` updated to the two-step flow. Post-merge: deploy 13 edge functions + `npx supabase db push --linked --yes` for the migration. |
