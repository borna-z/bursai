...
export function ChatWelcome({ mode, onSuggestion }: ChatWelcomeProps) {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center px-6 py-12 text-center">
      ...
      <p className="text-sm leading-relaxed text-muted-foreground max-w-xs whitespace-pre-wrap">
        {welcomeText}
      </p>
      <div className="flex flex-wrap gap-2 justify-center mt-6 max-w-sm">
        {suggestions.map((s) => (
          <button
            ...
            className="px-3.5 py-2 text-xs rounded-xl border border-border bg-background hover:bg-muted/60 text-foreground transition-colors"
          >
            {s}
          </button>
        ))}
