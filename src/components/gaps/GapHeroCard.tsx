...
export function GapHeroCard({ gap, garmentMap }: GapHeroCardProps) {
  ...
  return (
    <motion.article
      ...
      className="mt-2"
    >
      <p className="label-editorial text-accent/70">
        ...
        <div className="mt-5">
          ...
          <div className="mt-2 flex items-center gap-2">
            {pairingGarments.map((g) => (
              <div key={g.id} className="relative h-16 w-16 overflow-hidden rounded-[0.9rem] border border-border/40 bg-muted/30">
                <LazyImageSimple imagePath={getPreferredGarmentImagePath(g)} alt={g.title} className="h-full w-full object-cover" />
              </div>
            ))}
      ...
      <div className="mt-5 flex items-baseline gap-2">
        <span className="font-display italic text-[1.8rem] leading-none text-accent">+{gap.new_outfits}</span>
        <span className="text-[0.82rem] text-muted-foreground">{t('gaps.new_outfits') || 'new outfit combinations'}</span>
      </div>

      {gap.key_insight ? (
        <p className="mt-4 max-w-[32rem] font-display italic text-[13px] leading-6 text-foreground/55">{gap.key_insight}</p>
      ) : (
        <p className="mt-4 max-w-[32rem] text-[0.9rem] leading-6 text-muted-foreground">{gap.reason}</p>
      )}

      <div className="mt-5 flex flex-wrap items-center gap-2.5">
        <span className="rounded-full border border-border/40 bg-background/60 px-3 py-1.5 text-[0.78rem] text-muted-foreground">{gap.price_range}</span>
        <Button onClick={() => openGapSearchUrl(gap.search_query)} className="ml-auto rounded-full px-5">
