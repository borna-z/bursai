    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h2 className="mt-1 text-[1.18rem] font-semibold tracking-[-0.04em] text-foreground">
      ...
      {hasRefreshError ? (
        <div className="rounded-[1.2rem] border border-destructive/20 bg-destructive/5 px-4 py-3 text-[0.84rem] text-foreground">
      ...
      <motion.article
        ...
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.35, ease: EASE_CURVE }}
        className="relative overflow-hidden rounded-[1.65rem] border border-foreground/[0.08] bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(248,244,238,0.92))] p-4 shadow-[0_20px_48px_rgba(23,18,14,0.05)]"
      >
        ...
        <div className="space-y-3">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <h3 className="mt-2 text-[1.35rem] font-semibold tracking-[-0.055em] text-foreground">
                {featured.item}
              </h3>
            ...
            <div className="shrink-0 text-right">
              <p className="text-[2rem] font-semibold leading-none tracking-[-0.07em] text-foreground">
                +{featured.new_outfits}
              </p>
              <p className="mt-1 text-[0.68rem] uppercase tracking-[0.14em] text-muted-foreground/70">
                outfits
              </p>
            </div>
          </div>

          <p className="text-[0.88rem] leading-6 text-foreground/85">
            {featured.reason}
          </p>

          <div className="scrollbar-hide -mx-1 flex gap-2 overflow-x-auto px-1 text-[0.76rem] text-muted-foreground">
          ...
          <Button onClick={() => openGoogle(featured.search_query)} className="h-11 w-full justify-between rounded-full px-4">
            Search online
        ...
        <div className="space-y-3">
          ...
          {rest.map((gap, index) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <h3 className="truncate text-[0.98rem] font-semibold tracking-[-0.03em] text-foreground">
                    {gap.item}
                  </h3>
                ...
                <div className="text-right">
                  <div className="text-[1.12rem] font-semibold tracking-[-0.05em] text-foreground">
                    +{gap.new_outfits}
                  </div>
                  <div className="text-[0.68rem] text-muted-foreground">looks</div>
                </div>
              </div>

              <p className="mt-3 text-[0.84rem] leading-5 text-muted-foreground">
                {gap.reason}
              </p>

              <div className="mt-4 flex flex-wrap items-center justify-between gap-2">
                <span className="text-[0.78rem] text-muted-foreground">{gap.price_range}</span>
                <Button
