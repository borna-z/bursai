 ...
 *
 * Returns a stability progress (0–1), framing guidance, and fires
 * onStable() when the scene has been still for `stableMs` milliseconds.
 */
...
function computeEdgeDensity(data: Uint8ClampedArray): { overall: number; border: number } {
  const w = SAMPLE_SIZE;
  const h = SAMPLE_SIZE;
  let edgeCount = 0;
