// Outfit validation — thin wrapper over outfitRules that surfaces the
// "missing slots" + "is buildable" predicates the screens consume.
//
// Ported byte-for-byte from web `src/lib/outfitValidation.ts`. The wave's
// suggested `inferSlot` shorthand maps to `inferOutfitSlotFromGarment`
// (kept for web parity).
...
type BaseMissing = 'top' | 'bottom' | 'dress';
type CompleteMissing = BaseMissing | 'shoes' | 'outerwear';
...
export function canBuildCompleteOutfitPath<TGarment extends BasicGarmentLike>(
  garments: Pick<TGarment, 'category' | 'subcategory'>[],
): boolean {
  return hasCompleteOutfitPath(garments as Pick<OutfitRuleGarmentLike, 'category' | 'subcategory'>[]);
}
...
export function validateBaseOutfit<TGarment extends BasicGarmentLike>(items: OutfitValidationItem<TGarment>[]): OutfitValidationResult {
  ...
  return {
    ...
    missing: validation.missing.filter((slot): slot is BaseMissing => slot !== 'shoes'),
