// Outfit rules engine — slot inference + layering roles + validation.
//
// Ported byte-for-byte from web `src/lib/outfitRules.ts`. Pure functions, no
// DOM. Used by useGenerateOutfit (anchor enforcement post-response),
// outfitValidation (slot/missing helpers), and outfitAnchoring (lock-slot
// constraints).
//
// Web parity is the contract — keep this file in lockstep with the web copy.
// Slot constants stay lowercase ('top' | 'bottom' | …) per web convention;
// the wave file's UPPERCASE enum suggestion was not adopted because every
// existing call site (engine response items, outfit_items rows) uses
// lowercase.
...
export interface ValidateOutfitResult {
  ...
  missing: ('top' | 'bottom' | 'dress' | 'shoes')[];
...
export function collectCanonicalOutfitSlots(
  garments: Pick<OutfitRuleGarmentLike, 'category' | 'subcategory'>[],
): CanonicalOutfitSlot[] {
