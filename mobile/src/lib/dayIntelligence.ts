// Day-intelligence engine — ported byte-for-byte from `src/lib/dayIntelligence.ts`
// (web). Pure module: no React, no DB calls, no runtime imports from web.
// Mobile + web + the `summarize_day` edge function (`_shared/day-intelligence.ts`)
// all share the same scoring rules so a calendar/weather context produces the
// same `DayIntelligence` shape end-to-end.
//
// Inputs: a list of `DayEventInput`s (typically from calendar sync) + an
// optional `DayWeatherInput`. Output: a structured `DayIntelligence` describing
// the dominant occasion, formality, anchor event, and scored emphasis fields
// the outfit engine consumes downstream.
...
const OCCASION_RULES: { occasion: string; formality: DayFormality; confidence: number; tags: string[] }[] = [
...
/** Day-context shape consumed by the outfit-scoring layer (`buildSuggestions`).
 *  Holds the structured intelligence + the raw weather + a derived set of
 *  garment IDs the user has worn in the last N days (used to penalise repeats). */
export type DayContext = {
  intelligence: DayIntelligence;
  weather: DayWeatherInput | null;
  recentlyWornGarmentIds: Set<string>;
};

/** Lightweight weather descriptor used by the smart-day banner UI. Mirrors the
 *  shape the M35 weather hook will eventually produce; until then the mobile
 *  layer falls back to a mild placeholder (see `useSmartDayRecommendation`). */
export type DayWeatherContext = {
  temperature: number;
  /** Free-form short label — `'unknown'` when the placeholder kicks in. */
  condition: string;
  precipitation?: string;
};
