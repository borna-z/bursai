...
export function useNow(): Date {
  ...
  useEffect(() => {
    ...
    // Floor at 60s so DST boundaries (where setHours(24,...) can shorten the
    // day to 23h) plus a marginal-early fire can't schedule a sub-minute
    // timer that loops on the same day repeatedly. Codex P2 round on PR
    // #738.
    const ms = Math.max(60_000, next.getTime() - Date.now() + 1000);
