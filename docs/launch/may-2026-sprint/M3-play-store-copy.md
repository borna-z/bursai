...
| Account Deletion | Apps that allow account creation must allow in-app deletion AND offer a web-accessible deletion path. | In-app: Settings → Account → Delete Account (calls `delete_user_account` — shipped). Web: `burs.me/delete-account` — **does not currently ship** (`public/` has only `privacy/` + `terms/`); Borna must build it before Play submission. See `M3-privacy-declarations.md` "Borna pre-submission checklist" for the recommended minimal-compliant options. |
