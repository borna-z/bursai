...
> **Waves S + T opened in parallel (2026-05-15, supersedes the "queued" notes):** Wave S — [`docs/launch/waves/s-fast-smart-secure-ai.md`](waves/s-fast-smart-secure-ai.md) — split into 3 PRs: S-A security hardening (#849, branch `wave-s-a-security`), S-B structured output + context caching (#847, branch `wave-s-b-smartness`), S-C perceived-speed wins (#848, branch `wave-s-c-speed`). Wave T — [`docs/launch/waves/t-ai-speed-prefetch-streaming.md`](waves/t-ai-speed-prefetch-streaming.md) — opened as a PR (#850) on branch `prompt-ai-speed-four-improvements`, overriding the wave file's "push-only" note per direct user instruction. All four ride alongside #846 (tracker hygiene) under standing CEO post-launch authority. Merge order (recommended, not enforced): S-A → S-B → S-C → T. Per-PR functions to redeploy are enumerated in each PR body. Status (per-PR): see GitHub.
...
> **Campaign preemption (2026-05-09, retained for history):** M38 was paused for the six-defect G campaign (G6 → G2 → G1 → G5 → G4 → G3). All six PRs merged; M38–M42 resumed and shipped.
